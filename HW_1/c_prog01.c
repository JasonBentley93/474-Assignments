...
Printing:   Use the following functions to print:
  ...
  Resources used: www.w3schools.com, www.stackexchange.com, ChatGPT, www.geeksforgeeks.com
*/
...
void fill(shuffle deck[N_DECK][2]) {
    ...
    while (cardCounter < N_DECK)
    {
        ...
        for (i = 0; i < N_DECK; i++)
        {
            if ((deck[i][0] == number) && (deck[i][1] == suit))
            {
                cardExists = 1;
            }
        }
        ...
        if (!cardExists)
        {
            deck[cardCounter][0] = number;
            deck[cardCounter][1] = suit;
            cardCounter++;
        }
    }
...
int valid_card(unsigned char card){
    int cardInt = card;
    int min = convert(1,1);
    int max = convert (13,4);

    if (cardInt > max || cardInt <min)
    {
        return CARD_ERROR;
    }
...
void names(int card, int suit, char *answer) {
    ...
    memset(answer, '\0', sizeAnswer);
    
    int cNumber = card - 1;
    int cSuit   = suit - 1;

    int numLen  = strlen(card_names[cNumber]);
    int suitlen = strlen(suit_names[cSuit]);
    
    char *cardName = card_names[cNumber];
    char *cardSuit = suit_names[cSuit];
    ...
    int element_pos = numLen + 4;

    for (int i = 0; i < suitlen; i++)
    {
        answer[element_pos + i] = cardSuit[i];
    }
...
void printhand(int M, unsigned char* hand, char* buff1) {
    ...
    for (int i = 0; i < M; i++)
    {
        int cardToPrint = gcard(hand[i]);
        int suitToPrint = gsuit(hand[i]);
        names(cardToPrint, suitToPrint, buff1);
        print_str(buff1);
        print_newl();
    }
}
