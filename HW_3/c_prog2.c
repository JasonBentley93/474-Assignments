 ...
 *      
 *
 *      Student Name: Jason Bentley
 *      Student ID: 2177664
 *
 *      WRITE YOUR CODE IN THIS FILE
 * 
 *      AKNOWLEDGMENT: ChatGPT, www.scaler.com (Prateek Narang), www.tutorialspoint.com, www.cplusplus.com
 *
 */
...
int  bit_check(int data, int bits_on, int bits_off){
    if (bits_off & bits_on)  //if any of the bits_off and bits_on are the same
        return -1;
    
    if ((data & bits_on) == bits_on){ // if all of the bits_on bits are on in data
        if(!(data & bits_off)) // if all of the bits_off bits are off in data
            return 1;
    }
...
char* pmatch(char c){
    int a_array_size = sizeof(a_array) / sizeof(char); // get size of array

    // parse array and check if char c exists, if it exists return the address of that array element
    for(int i = 0; i < a_array_size; i++){
        if(a_array[i] == c)
            return &a_array[i];
    }

    return NULL; // if exited for loop with no match then return NULL
}
...
int ldif(char c1, char c2){
    if (pmatch(c1) == NULL || pmatch(c2) == NULL) //if either chars to not exist
        return -27;
    
    return abs(pmatch(c1) - pmatch(c2)); //absolute value of differnece
}
 ...
 *  -----------------------
 */
void floats_to_ints(float f, int* output){
  output[0] = (int) f; //extract portion before decimal
  output[1] = (int) ( (f-output[0]) * 100); //extract first two decimal places
}


char* per_print(Person* p, char* personbuf) {
    char trunc_addr[61];  //truncate all address using this array
    int height_to_int[2]; //store both sides of height decimal
    int weight_to_int[2]; //store both sides of weight decimal
 
    strncpy(trunc_addr, p->StreetAddr, 60); //truncate all addresses
    trunc_addr[60] = '\0';

    floats_to_ints(p->Height, height_to_int);
    floats_to_ints(p->Weight, weight_to_int);
    ...
    sprintf(personbuf, 
        ...
        trunc_addr,
        p->ZipCode,
        height_to_int[0], height_to_int[1],
        weight_to_int[0], weight_to_int[1],
        p->DBirth);
